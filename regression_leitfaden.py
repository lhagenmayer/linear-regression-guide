...
# ---------------------------------------------------------
# R-OUTPUT FIGURE HELPER FUNCTIONS (Aufgeteilte Darstellung)
# ---------------------------------------------------------
def _draw_residuals_section(ax, q, y_pos, mono_props, bold_mono):
    """Zeichnet den Residuals-Abschnitt der R-Ausgabe."""
    ax.text(5, y_pos, "Residuals:", **bold_mono, color='#8B0000')
    y_pos -= 4
    ax.text(5, y_pos, "    Min      1Q  Median      3Q     Max", **mono_props)
    y_pos -= 4
    ax.text(5, y_pos, f"{q[0]:7.4f} {q[1]:7.4f} {q[2]:7.4f} {q[3]:7.4f} {q[4]:7.4f}", **mono_props)
    
    # Annotation
    ax.add_patch(FancyBboxPatch((90, 88), 26, 8,
                                boxstyle="round,pad=0.02", facecolor='#ffcccc', edgecolor='red', linewidth=1.5))
    ax.text(103, 92, "Deskriptive\nStatistiken", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(35, y_pos+3), xytext=(90, 90),
                arrowprops=dict(arrowstyle='->', color='red', lw=1.2, connectionstyle="arc3,rad=0.15"))
    return y_pos - 6

def _draw_coefficients_section(ax, params, bse, tvals, pvals, feature_name, y_pos, mono_props, bold_mono):
    """Zeichnet den Coefficients-Abschnitt der R-Ausgabe."""
    ax.text(5, y_pos, "Coefficients:", **bold_mono, color='#8B0000')
    y_pos -= 4
    ax.text(5, y_pos, f"{'':13} {'Estimate':>9} {'Std.Err':>8} {'t val':>7} {'Pr(>|t|)':>10}", **mono_props)
    y_pos -= 4
    
    # Intercept Row
    p0 = pvals[0]
    stars0 = get_signif_stars(p0)
    ax.text(5, y_pos, f"{'(Intercept)':<13} {params[0]:9.4f} {bse[0]:8.4f} {tvals[0]:7.2f} {p0:10.4g} {stars0}", 
            **mono_props, color='#333')
    y_pos -= 4
    
    # Feature Row
    p1 = pvals[1]
    stars1 = get_signif_stars(p1)
    ax.text(5, y_pos, f"{feature_name:<13} {params[1]:9.4f} {bse[1]:8.4f} {tvals[1]:7.2f} {p1:10.4g} {stars1}", 
            **mono_props, color='#333')
    
    coef_y = y_pos + 6
    return y_pos, coef_y

def _draw_annotations_section(ax, coef_y):
    """Zeichnet die Annotations-Boxen rechts neben der R-Ausgabe."""
    # Spalte 1: Estimates
    ax.add_patch(FancyBboxPatch((64, 74), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#ffe6e6', edgecolor='red', linewidth=1.5))
    ax.text(76, 78, r"$b_0 = \bar{y} - b_1\bar{x}$", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(18, coef_y+2), xytext=(64, 76),
                arrowprops=dict(arrowstyle='->', color='red', lw=1.2, connectionstyle="arc3,rad=0.1"))
    
    ax.add_patch(FancyBboxPatch((64, 63), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#ffe6e6', edgecolor='red', linewidth=1.5))
    ax.text(76, 67, r"$b_1 = \frac{cov(x,y)}{s_x^2}$", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(18, coef_y-2), xytext=(64, 65),
                arrowprops=dict(arrowstyle='->', color='red', lw=1.2, connectionstyle="arc3,rad=-0.05"))
    
    # Spalte 2: Std. Errors
    ax.add_patch(FancyBboxPatch((92, 74), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#e6f3ff', edgecolor='blue', linewidth=1.5))
    ax.text(104, 78, r"$s_{b_0}$ Formel", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(30, coef_y+2), xytext=(92, 76),
                arrowprops=dict(arrowstyle='->', color='blue', lw=1.2, connectionstyle="arc3,rad=0.15"))
    
    ax.add_patch(FancyBboxPatch((92, 63), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#e6f3ff', edgecolor='blue', linewidth=1.5))
    ax.text(104, 67, r"$s_{b_1} = \frac{s_e}{\sqrt{SS_x}}$", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(30, coef_y-2), xytext=(92, 65),
                arrowprops=dict(arrowstyle='->', color='blue', lw=1.2, connectionstyle="arc3,rad=-0.1"))
    
    # t-Tests
    ax.add_patch(FancyBboxPatch((64, 50), 24, 10,
                                boxstyle="round,pad=0.02", facecolor='#e6ffe6', edgecolor='green', linewidth=1.5))
    ax.text(76, 55, r"$t_0 = \frac{b_0}{s_{b_0}}$" + "\n" + r"$t_1 = \frac{b_1}{s_{b_1}}$", 
            fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(42, coef_y), xytext=(64, 53),
                arrowprops=dict(arrowstyle='->', color='green', lw=1.2, connectionstyle="arc3,rad=-0.15"))
    
    # p-Werte
    ax.add_patch(FancyBboxPatch((92, 50), 24, 10,
                                boxstyle="round,pad=0.02", facecolor='#fff0f5', edgecolor='purple', linewidth=1.5))
    ax.text(104, 55, "p-Werte t-Tests\n(zweiseitig)", fontsize=9, ha='center', va='center', color='purple')
    ax.annotate("", xy=(52, coef_y), xytext=(92, 53),
                arrowprops=dict(arrowstyle='->', color='purple', lw=1.2, connectionstyle="arc3,rad=-0.2"))
    
    # Signif codes
    ax.add_patch(FancyBboxPatch((92, 35), 24, 12,
                                boxstyle="round,pad=0.02", facecolor='#fff5ee', 
                                edgecolor='#8B4513', linewidth=1.5, linestyle='dashed'))
    ax.text(104, 41, "Signifikanz-Codes:\n*** p<0.001\n** p<0.01\n* p<0.05", 
            fontsize=7, ha='center', va='center', color='#8B4513')

def _draw_model_statistics_section(ax, model, rse, df_resid, df_model, y_pos, mono_props):
    """Zeichnet den Model Statistics-Abschnitt der R-Ausgabe."""
    ax.text(5, y_pos, f"Residual standard error: {rse:.4f} on {df_resid} degrees of freedom", 
            **mono_props, color='#8B0000')
    
    # se Annotation
    ax.add_patch(FancyBboxPatch((64, 28), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#ffe6e6', edgecolor='red', linewidth=1.5))
    ax.text(76, 32, r"$s_e = \sqrt{\frac{SSE}{n-2}}$", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(28, y_pos), xytext=(64, 30),
                arrowprops=dict(arrowstyle='->', color='red', lw=1.2, connectionstyle="arc3,rad=0.1"))
    
    y_pos -= 4
    ax.text(5, y_pos, f"Multiple R-squared:  {model.rsquared:.4f},    Adjusted R-squared:  {model.rsquared_adj:.4f}", 
            **mono_props, color='#006400')
    
    # R² Annotation
    ax.add_patch(FancyBboxPatch((64, 16), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#e6ffe6', edgecolor='green', linewidth=1.5))
    ax.text(76, 20, r"$R^2 = \frac{SSR}{SST} = 1 - \frac{SSE}{SST}$", fontsize=8, ha='center', va='center')
    ax.annotate("", xy=(23, y_pos), xytext=(64, 18),
                arrowprops=dict(arrowstyle='->', color='green', lw=1.2, connectionstyle="arc3,rad=0.1"))
    
    y_pos -= 4
    ax.text(5, y_pos, f"F-statistic: {model.fvalue:.1f} on {df_model} and {df_resid} DF,  p-value: {model.f_pvalue:.4g}", 
            **mono_props, color='#00008B')
    
    # F-Test Annotation
    ax.add_patch(FancyBboxPatch((92, 16), 24, 8,
                                boxstyle="round,pad=0.02", facecolor='#e6e6ff', edgecolor='blue', linewidth=1.5))
    ax.text(104, 20, r"$F = \frac{MSR}{MSE}$", fontsize=9, ha='center', va='center')
    ax.annotate("", xy=(18, y_pos), xytext=(92, 18),
                arrowprops=dict(arrowstyle='->', color='blue', lw=1.2, connectionstyle="arc3,rad=0.15"))
    return y_pos
...
def create_r_output_figure(model, feature_name="X", figsize=(16, 12)):
    """
    Erstellt eine Matplotlib-Figur die exakt wie R's summary(lm) aussieht.
    Nutzt Helper-Funktionen fuer die einzelnen Abschnitte.
    """
    ...
    # Hintergrund-Box
    main_box = FancyBboxPatch((3, 10), 58, 80, 
                               boxstyle="round,pad=0.02,rounding_size=0.5",
                               facecolor='#f5f5f5', edgecolor='#333333', linewidth=2)
    ax.add_patch(main_box)
    
    # Font-Styles
    mono_props = dict(fontfamily='monospace', fontsize=10)
    bold_mono = dict(fontfamily='monospace', fontsize=10, fontweight='bold')
    
    # Zeichne alle Abschnitte
    y_pos = 87
    y_pos = _draw_residuals_section(ax, q, y_pos, mono_props, bold_mono)
    y_pos, coef_y = _draw_coefficients_section(ax, params, bse, tvals, pvals, feature_name, y_pos, mono_props, bold_mono)
    _draw_annotations_section(ax, coef_y)
    
    y_pos -= 3
    ax.text(5, y_pos, "---", **mono_props)
    y_pos -= 4
    ax.text(5, y_pos, "Signif. codes: 0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1", 
            fontfamily='monospace', fontsize=8, color='#666')
    y_pos -= 5
    
    _draw_model_statistics_section(ax, model, rse, df_resid, df_model, y_pos, mono_props)
    
    # Title
    ax.text(60, 96, "Python Replikation des R-Outputs: summary(lm_model)", 
...
if regression_type == "📊 Multiple Regression":
    with st.spinner("Lade Datensatz..."):
        ...
        if dataset_choice_mult == "🏙️ Städte-Umsatzstudie (75 Städte)":
            ...
            noise_mult = np.random.normal(0, 3.5, n_mult)
...
if regression_type == "📈 Einfache Regression":
    ...
    if dataset_choice == "🏠 Häuserpreise mit Pool (1000 Häuser)":
        ...
        # Für einfache Regression: Wähle nur eine Variable
...
# Nur bei Einfacher Regression: Zeige die bestehenden Kapitel
    ...
    **Beobachtung:** Die Punkte scheinen einem aufsteigenden Trend zu folgen! 
    ...
    with col_cov1:
        ...
        if show_3d_cov:
            ...
            ax_cov.axhline(y_mean_val_local, color='orange', linestyle='--', linewidth=2, label=f'ȳ = {y_mean_val_local:.2f}')
            ax_cov.axvline(x_mean_val, color='green', linestyle='--', linewidth=2, label=f'x̄ = {x_mean_val:.2f}')
        
            # Quadranten-Hintergrund
            ax_cov.fill_between([min(x)-0.5, x_mean_val], [y_mean_val_local, y_mean_val_local], [max(y)+0.5, max(y)+0.5], 
                                alpha=0.1, color='red', label='Quadrant II (−)')
            ax_cov.fill_between([x_mean_val, max(x)+0.5], [y_mean_val_local, y_mean_val_local], [max(y)+0.5, max(y)+0.5], 
                                alpha=0.1, color='green', label='Quadrant I (+)')
            ax_cov.fill_between([min(x)-0.5, x_mean_val], [min(y)-0.5, min(y)-0.5], [y_mean_val_local, y_mean_val_local], 
                                alpha=0.1, color='green')
            ax_cov.fill_between([x_mean_val, max(x)+0.5], [min(y)-0.5, min(y)-0.5], [y_mean_val_local, y_mean_val_local], 
                                alpha=0.1, color='red')
            ...
            for i in range(len(x)):
                ...
                rect = plt.Rectangle((x_mean_val, y_mean_val_local), dx, dy, 
                                     alpha=0.15, facecolor=color, edgecolor=color, linewidth=1)
                ax_cov.add_patch(rect)
            
                # Datenpunkt
                ax_cov.scatter(x[i], y[i], s=100, c=color, edgecolor='white', linewidth=2, zorder=5)
        
            ax_cov.scatter([x_mean_val], [y_mean_val_local], s=200, c='black', marker='X', zorder=10, label='Schwerpunkt')
            ax_cov.set_xlabel(f'{x_label} (X)', fontsize=12)
            ax_cov.set_ylabel(f'{y_label} (Y)', fontsize=12)
            ax_cov.set_title('Kovarianz-Visualisierung: Grüne Rechtecke addieren, rote subtrahieren', 
                            fontsize=13, fontweight='bold')
            ax_cov.legend(loc='upper left')
            ax_cov.grid(True, alpha=0.3)
            ax_cov.set_xlim(min(x) - 0.5, max(x) + 0.5)
            ax_cov.set_ylim(min(y) - 0.5, max(y) + 0.5)
    ...
    with col_corr1:
        # Verschiedene Korrelationen zeigen
        fig_corr_examples, axes = plt.subplots(2, 3, figsize=(14, 8))
        ...
        for idx, r in enumerate(example_corrs):
            ...
            ax.scatter(ex_x, ex_y, alpha=0.6, c=[color], s=30)
            ax.set_title(f'r = {r:.2f}', fontsize=14, fontweight='bold', 
            ...
            if r != 0:
                try:
                    with warnings.catch_warnings():
                        ...
                        p = np.poly1d(z)
                        ax.plot([-3, 3], [p(-3), p(3)], 'k--', alpha=0.5)
                except (np.linalg.LinAlgError, ValueError):
                    pass  # Bei problematischen Daten keine Linie zeichnen
    
        plt.suptitle('Der Korrelationskoeffizient r: Von -1 (perfekt negativ) bis +1 (perfekt positiv)', 
    ...
    **Das Kernprinzip:** Wähle jene Gerade, welche die **Summe der quadrierten vertikalen Abweichungen** 
    ...
    if show_3d_detail:
        ...
        ax_detail.plot([x_start, x_end], [y_start, y_start], 'k--', linewidth=2)
        # Vertikale Linie (Δy)
        ax_detail.plot([x_end, x_end], [y_start, y_end], 'k--', linewidth=2)
        # Beschriftung
        ax_detail.annotate('', xy=(x_end-0.1, y_start), xytext=(x_start+0.1, y_start),
                          arrowprops=dict(arrowstyle='<->', color='black', lw=1.5))
        ax_detail.text((x_start + x_end)/2, y_start - 0.4, r'$\Delta x = 2$', fontsize=12, ha='center')
        ax_detail.annotate('', xy=(x_end, y_end-0.1), xytext=(x_end, y_start+0.1),
                          arrowprops=dict(arrowstyle='<->', color='black', lw=1.5))
        ax_detail.text(x_end + 0.3, (y_start + y_end)/2, rf'$\Delta y = {b1*2:.2f}$', fontsize=12, va='center')
        ax_detail.text(x_end + 0.3, y_start + 0.3, rf'$b_1 = \frac{{\Delta y}}{{\Delta x}} = {b1:.2f}$', fontsize=11, 
        ...
        ax_detail.plot([x[idx_eps], x[idx_eps]], [y[idx_eps], y_pred[idx_eps]], 'r-', linewidth=2.5)
        # Annotation mit Pfeil
        mid_y = (y[idx_eps] + y_pred[idx_eps]) / 2
        ax_detail.annotate(r'$\epsilon_i$ = ' + f'{residuals[idx_eps]:.2f}', 
                          xy=(x[idx_eps], mid_y), 
                          xytext=(x[idx_eps] + 1, mid_y + 0.5),
                          fontsize=12, color='red',
                          arrowprops=dict(arrowstyle='->', color='red', lw=1.5),
                          bbox=dict(boxstyle='round', facecolor='mistyrose', alpha=0.9))
    
        # 6. Schwerpunkt markieren
        ax_detail.scatter([x_mean], [y_mean_val], s=200, c='orange', marker='*', edgecolor='black', 
                         linewidth=2, zorder=5, label=f'Schwerpunkt (x̄, ȳ) = ({x_mean:.1f}, {y_mean_val:.1f})')
    
        ax_detail.set_xlabel(x_label, fontsize=12)
        ax_detail.set_ylabel(y_label, fontsize=12)
        ax_detail.set_title('Anatomie des Regressionsmodells: Steigung, Fehlerterm & Konfidenzintervall', 
                           fontsize=14, fontweight='bold')
    ...
    **Das Konfidenzintervall (hellblauer Bereich):**
    ...
    with col_r2_1:
        ...
        if show_3d_var:
            ...
            # SSR
            axes[1].scatter(x, y, color='gray', alpha=0.3, s=60)
            axes[1].axhline(y_mean_val, color='orange', linewidth=2, linestyle='--', alpha=0.5)
            axes[1].plot(x, y_pred, color='blue', linewidth=3)
            for i in range(len(x)):
                axes[1].plot([x[i], x[i]], [y_pred[i], y_mean_val], color='green', alpha=0.6, linewidth=2)
            axes[1].set_title(f'SSR = {ssr:.2f}\n(Durch Modell erklärt)', fontsize=12, fontweight='bold', color='green')
            axes[1].set_xlabel('X')
            axes[1].grid(True, alpha=0.3)
        
            # SSE
            axes[2].scatter(x, y, color='gray', alpha=0.6, s=60)
            axes[2].plot(x, y_pred, color='blue', linewidth=3)
            for i in range(len(x)):
                axes[2].plot([x[i], x[i]], [y[i], y_pred[i]], color='red', alpha=0.6, linewidth=2)
            axes[2].set_title(f'SSE = {sse:.2f}\n(Unerklärt/Residuen)', fontsize=12, fontweight='bold', color='red')
            axes[2].set_xlabel('X')
            axes[2].grid(True, alpha=0.3)
        
            plt.suptitle('Die Zerlegung der Varianz: SST = SSR + SSE', fontsize=14, fontweight='bold')
            plt.tight_layout()
            st.pyplot(fig_var)
            plt.close()
    ...
    with col_r2_2:
        ...
        # R² als Balken
        fig_r2bar, ax_r2 = plt.subplots(figsize=(6, 4))
        bars = ax_r2.bar(['SST\n(Total)', 'SSR\n(Erklärt)', 'SSE\n(Unerklärt)'], 
                         [sst, ssr, sse], color=['gray', 'green', 'red'], alpha=0.7, edgecolor='black')
    ...
    axes[0, 0].scatter(x_demo, y_correct_1, alpha=0.5, c='green', s=30)
    axes[0, 0].plot(x_demo, 2 + 0.5 * x_demo, 'b-', linewidth=2)
    axes[0, 0].axhline(2 + 0.5 * 5.5, color='gray', linestyle='--', alpha=0.5)
    axes[0, 0].set_title('✅ (1) E(εᵢ|xᵢ) = 0\nFehler symmetrisch um Null', fontweight='bold', color='green')
    ...
    axes[0, 1].scatter(x_demo, y_violated_1, alpha=0.5, c='red', s=30)
    axes[0, 1].plot(x_demo, 2 + 0.5 * x_demo, 'b-', linewidth=2, label='Lineares Modell')
    axes[0, 1].plot(x_demo, 2 + 0.5 * x_demo + 0.1 * (x_demo - 5)**2, 'g--', linewidth=2, label='Wahrer Zusammenhang')
    axes[0, 1].set_title('❌ E(εᵢ|xᵢ) ≠ 0\nNicht-linearer Zusammenhang ignoriert', fontweight='bold', color='red')
    ...
    axes[1, 0].scatter(x_demo, y_correct_2, alpha=0.5, c='green', s=30)
    axes[1, 0].plot(x_demo, 2 + 0.5 * x_demo, 'b-', linewidth=2)
    axes[1, 0].fill_between(x_demo, 2 + 0.5 * x_demo - 2, 2 + 0.5 * x_demo + 2, alpha=0.2, color='blue')
    axes[1, 0].set_title('✅ (2) Var(εᵢ) = σ² (konstant)\nHomoskedastizität', fontweight='bold', color='green')
    ...
    axes[1, 1].scatter(x_demo, y_violated_2, alpha=0.5, c='red', s=30)
    axes[1, 1].plot(x_demo, 2 + 0.5 * x_demo, 'b-', linewidth=2)
    axes[1, 1].fill_between(x_demo, 2 + 0.5 * x_demo - 0.6 * x_demo, 2 + 0.5 * x_demo + 0.6 * x_demo, alpha=0.2, color='red')
    axes[1, 1].set_title('❌ Var(εᵢ|xᵢ) = f(xᵢ)\nHeteroskedastizität (Trichter)', fontweight='bold', color='red')
    ...
    axes[2, 0].scatter(range(n_demo-1), resid_correct_3[:-1], c=resid_correct_3[1:], cmap='coolwarm', s=30, alpha=0.7)
    axes[2, 0].axhline(0, color='gray', linestyle='--')
    axes[2, 0].set_title('✅ (3) Cov(εᵢ, εⱼ) = 0\nKeine Autokorrelation', fontweight='bold', color='green')
    axes[2, 0].set_xlabel('Beobachtung i')
    axes[2, 0].set_ylabel('Residuum eᵢ')
    ...
    axes[2, 1].plot(range(n_demo), auto_error, 'r-', alpha=0.7, linewidth=1.5)
    axes[2, 1].scatter(range(n_demo), auto_error, c='red', s=20, alpha=0.5)
    axes[2, 1].axhline(0, color='gray', linestyle='--')
    axes[2, 1].set_title('❌ Cov(εᵢ, εⱼ) ≠ 0\nAutokorrelation (Muster in Residuen)', fontweight='bold', color='red')
    axes[2, 1].set_xlabel('Zeit/Beobachtung')
    ...
    axes[3, 0].hist(normal_resid, bins=20, density=True, alpha=0.7, color='green', edgecolor='black')
    x_norm = np.linspace(-4, 4, 100)
    axes[3, 0].plot(x_norm, stats.norm.pdf(x_norm, 0, 1), 'b-', linewidth=2)
    axes[3, 0].set_title('✅ (4) ε ~ N(0, σ²)\nNormalverteilte Residuen', fontweight='bold', color='green')
    axes[3, 0].set_xlabel('Residuum')
    axes[3, 0].set_ylabel('Dichte')

    # Verletzt: Schiefe/Nicht-normale Verteilung
    skewed_resid = np.random.exponential(1, n_demo) - 1  # Rechtsschiefe Verteilung
    axes[3, 1].hist(skewed_resid, bins=20, density=True, alpha=0.7, color='red', edgecolor='black')
    axes[3, 1].plot(x_norm, stats.norm.pdf(x_norm, 0, 1), 'b--', linewidth=2, label='Normalverteilung')
    axes[3, 1].set_title('❌ ε nicht normalverteilt\nSchiefe Verteilung', fontweight='bold', color='red')
    axes[3, 1].set_xlabel('Residuum')
    axes[3, 1].legend(fontsize=8)

    plt.suptitle('Die 4 Gauss-Markov Annahmen: Korrekt (links) vs. Verletzt (rechts)', fontsize=16, fontweight='bold', y=1.01)
    plt.tight_layout()
    ...
    st.pyplot(fig_r_output)
    plt.close(fig_r_output)
