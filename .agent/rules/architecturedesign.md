...
- **Dependency Rule**: Dependencies always point inward (UI → Application → Domain ← Infrastructure)
...
class RegressionModel:
    def predict(self, data: Dict[str, np.ndarray]) -> np.ndarray:
        # ❌ WRONG: Can't use numpy in domain layer
        pass
```
...
1. **Business Rules/Logic** → Domain Layer
   - Validation rules, calculations, constraints
   - Domain entities, value objects, services

2. **Workflow Orchestration** → Application Layer
   - Use cases coordinating multiple domain objects
   - CQRS command/query handling
   - Application services

3. **External Integrations** → Infrastructure Layer
   - Database operations, API calls, UI rendering
   - Framework-specific implementations
   - File system operations

4. **Configuration/Setup** → Config Layer
...
### 🚫 NEVER DO THIS:
```python
# ❌ Domain layer importing external frameworks
