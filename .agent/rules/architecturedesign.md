...
│   │   ├── value_objects.py# RegressionParameters, RegressionMetrics
...
│   ├── data/
│   │   ├── generators.py   # DataFetcher (numpy)
│   │   └── provider.py     # DataProviderImpl implements IDataProvider
│   ├── services/
│   │   ├── calculate.py    # StatisticsCalculator (scipy)
│   │   ├── plot.py         # PlotBuilder (plotly)
│   │   ├── regression.py   # RegressionServiceImpl implements IRegressionService
│   │   ├── classification.py # ClassificationServiceImpl (Pure NumPy)
│   │   └── data_splitting.py # DataSplitterService
│   ├── content/            # Educational content builder
│   └── ai/                 # Perplexity client & ROutputFormatter
│
├── api/                    # REST API Layer
│   ├── endpoints.py        # RegressionAPI, ContentAPI, AIInterpretationAPI
...
│
├── container.py            # DI Container (wires implementations)
